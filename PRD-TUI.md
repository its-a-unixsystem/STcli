...
- **Provider/Preset Picker:** The Prompt Preset picker opens with `P` from Sessions or Chat. It supports `i` import, `/` substring filtering, and a `d`/`Tab` side-by-side inspector for prompt order, generation parameters, and inert embedded scripts. When the inspector is open, `PgDn`/`PgUp` scroll its contents (page by page), `Shift+Down`/`Shift+Up` scroll line by line, and the mouse wheel scrolls the inspector; the scroll position clamps to the content bounds and resets when the selected preset changes. `Enter` applies a preset in Chat or closes the picker on Sessions; `n` chains from Sessions into New Session with the highlighted preset.
