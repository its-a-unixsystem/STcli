...
- **Provider Profile Creator:** Form modal for creating and persisting provider profiles into `config.toml`. It includes a template selector, URL, model, Chat Completions path, stream settings, and a credential-source selector. Credential Store mode writes masked secret input to the platform keyring and persists only a Credential Reference; Environment Variable mode persists `api_key_env`. Edit mode reports an existing Credential Reference without retrieving the secret.
...
- **Generation Settings Modal:** In Chat history, `s` opens a form prefilled from the active Session's effective reasoning effort, temperature, and maximum response tokens. Reasoning effort supports omit, `low`, `medium`, `high`, and custom values. Saving appends a Session Configuration Revision so regeneration uses the updated settings.
- **Preset Clone Modal:** The preset copy form edits name, temperature, reasoning effort, context and response token limits, and system-prompt use before importing a new immutable Artifact Revision.
